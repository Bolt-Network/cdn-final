...
class Game {
    ...
    updateContinueButton() {
        if (this.loadProgress()) {
            ...
            this.continueBtn.textContent = `继续 (第 ${this.state.currentNight} 夜)`;
        } else {
    ...
    showTutorial(type = 'night1') {
        ...
        if (type === 'night2') {
            ...
            tutorialContent.innerHTML = `
                <h2>如何防御特朗普</h2>
                <p>
                    特朗普会试图通过 CAM 1 和 CAM 2 的通风管道攻击你，所以如果你听到通风管道里有撞击声，请前往控制面板关闭通风口。
                    关闭通风口后，几秒钟后你会再次听到撞击声，这意味着他已经离开了通风管道。你必须重新打开通风口，否则你会因缺氧而死亡。
                    特朗普可以被声音诱饵吸引，但你使用声音诱饵的首要目标应该是爱泼斯坦。
                </p>
                <button id="tutorial-got-it">明白了</button>
            `;
            // 重新绑定按钮事件
            ...
            tutorialContent.innerHTML = `
                <h2>如何防御史蒂芬·霍金</h2>
                <p>
                    史蒂芬·霍金始终待在 CAM 6，且不受声音诱饵的影响。
                    你需要每隔一段时间电击霍金，以防止他离开 CAM 6。
                </p>
                <button id="tutorial-got-it">明白了</button>
            `;
            // 重新绑定按钮事件
            ...
            tutorialContent.innerHTML = `
                <h2>如何防御爱泼斯坦</h2>
                <p>
                    爱泼斯坦总是从 CAM 11 出发。使用摄像头的声音诱饵让爱泼斯坦远离你。
                    确保你播放声音的摄像头与爱泼斯坦所在的摄像头相邻。
                    在同一个位置连续播放两次以上的声音是无效的。
                    过度使用声音诱饵会导致摄像头系统崩溃。
                    要修复摄像头，请前往控制面板重启摄像头系统。
                    爱泼斯坦不会通过通风管道攻击，所以今晚不需要关闭通风口。
                </p>
                <button id="tutorial-got-it">明白了</button>
            `;
            // 重新绑定按钮事件
    ...
    showNightIntro() {
        return new Promise((resolve) => {
            ...
            if (this.state.customNight && this.state.currentNight === 7) {
                nightIntroText.textContent = '自定义夜晚';
            } else {
                nightIntroText.textContent = `第 ${this.state.currentNight} 夜`;
            }
    ...
    playNightEndAnimation() {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                // Custom Night 通关显示
                if (this.state.customNight && this.state.currentNight === 7) {
                    timeDisplay.textContent = '自定义夜晚已通关';
                ...
                // 如果还有下一关，显示剩余天数（故事设定是5晚，所以总是显示5-当前关卡）
                    ...
                    timeDisplay.textContent = `距离救援还有 ${daysRemaining} 天`;
                    timeDisplay.style.fontSize = '5vw';
                    timeDisplay.style.color = '#fff';
                } else {
                    // 所有关卡完成，显示TO BE CONTINUED
                    timeDisplay.innerHTML = '未完待续...<br><span style="font-size: 3vw; color: #f00;">网页版移植中</span>';
                    timeDisplay.style.fontSize = '5vw';
                    timeDisplay.style.color = '#fff';
                }
    ...
    gameOverScreen(message, win = false) {
        ...
        if (win) {
            ...
            if (this.state.currentNight < this.state.maxNights) {
                ...
                subtitle.textContent = '未完待续...（网页版移植中）';
